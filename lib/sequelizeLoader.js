...
module.exports = app => {
  ...
  function loadDatabase(config = {}) {
    ...
    const modelDirList = [ path.join(__dirname, '../app/model') ];
    // 项目model目录
    const projectModelDir = path.join(app.baseDir, 'app', config.baseDir);
    // 检查目录是否存在，如果存在则使用项目中的model目录
    if (fs.existsSync(projectModelDir)) modelDirList.push(projectModelDir);
    const modelDir = modelDirList[modelDirList.length - 1];
